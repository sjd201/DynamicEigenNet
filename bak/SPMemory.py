...
class SPMemory():
  ...
  def getTraceActivationsFromHistory(self, endofframeword, indexinframe=None):
    ...
    for index in xrange(len(self.TraceActivationHistory)-1, 0, -1):
    ...
    result += "\n"
    for l in xrange(len(self.TraceActivationHistory)-100, len(self.TraceActivationHistory)):
      result += self.TraceActivationHistory[l][0] + " "
    result += "\n"
    start = max(0, index-SPVec.NumberOfSlots)
    result +=  " ".join(t[0] for t in self.TraceActivationHistory[start:(index+1)]) + "\n"
 
    if indexinframe != None:
      word = self.TraceActivationHistory[index - SPVec.NumberOfSlots + indexinframe + 1][0]
      result += word + "\n"
    ...
    if self.TraceActivationHistory[index][2].nnz > 0:
      ...
      for i in xrange(len(res.indices)):
        realind = res.indices[i]
        if res.data[i] > 0.01 and i < 10:
          result +=  "%1.3f" % res.data[i] + " "+ str(SPVec(self.M[realind])) + "\n"
    else:
  ...
  def retrieveFromMemory(self, probe, scale = 1.0, verbose=False):
    '''
    ...
    self.topn()
    self.TraceActivations = self.TraceActivations * scale
  ...
  def getEcho(self, vec, scale = 1.0, verbose=False):
    self.retrieveFromMemory(vec, scale, verbose=verbose)
    echo = self.TraceActivations * self.M
    return SPVec(echo.tolil())
      ...
      if self.TraceActivations.nnz > 0:
        ...
        for i in xrange(len(res.indices)):
          ...
          if res.data[i] > 0.1 or i < 4:
            if showTraceNumber:
              print "%4d " % realind, SPVec(self.M[realind]),
            if corpus:
              print "%1.7f" % res.data[i], " ".join(corpus[realind:(realind+SPVec.NumberOfSlots)])
            else:
              print "%1.7f" % res.data[i], SPVec(self.M[realind])
      else:
        print "TraceActivations has %d non zeros" % self.TraceActivations.nnz

  def showTraces(self, corpus=None):
      for i in xrange(self.NumberOfTraces):
        if corpus:
          print " ".join(corpus[realind:(realind+SPVec.NumberOfSlots)])
        else:
          print SPVec(self.M[i])

  def listTraces(self, corpus):
    for i in xrange(self.NumberOfTraces):
      print "%4d %s" % (i, " ".join(corpus[i:(i+SPVec.NumberOfSlots)]))
      print "%4d" % i, SPVec(self.M[i])
